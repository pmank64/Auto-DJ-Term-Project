...
void checkIfPresent(Library* lib){
    ...
    for (int i = 0;i<50;i++){
        if(lib->getSongInfo("title" + std::to_string(i),"artist" + std::to_string(i))=="This song is not in the library"){
...
void createPlaylists(Library* lib){
    ...
    for (int j =0;j<50;j++){
        for (int n =0;n<10;n++){
            lib->addSongToPlaylist("playlist"+std::to_string(j),"artist"+std::to_string(n),"title"+std::to_string(n));
        }
    }
}
...
//    std::cout << lib->listSongs() << std::endl;
